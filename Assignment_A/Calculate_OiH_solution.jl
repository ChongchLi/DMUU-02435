...
function Calculate_OiH_solution(day1_price, day2_price)
    ...
    model_OiH = Model(Gurobi.Optimizer) # declare model with Gurobi
    ...
    # objective function
    @objective(model_OiH, Min, sum(price_coffee[t] * x_order[w, t] for w in 1:number_of_warehouses, t in 1:number_of_simulation_periods)
    ...
    # inventory balance
    @constraint(model_OiH, inventory_balance[w in 1:number_of_warehouses, t in 2:number_of_simulation_periods], demand_coffee[w,t] == z_storage[w, t-1] + z_storage[w, t] + x_order[w,t] + sum(y_received[w,q,t] - y_send[w,q,t] for q in 1:number_of_warehouses) + m_missing[w,t])
    # initial balance
    @constraint(model_OiH, inventory_balance_start[w in 1:number_of_warehouses], demand_coffee[w,1] == initial_stock[w] + z_storage[w, 1] + x_order[w,1] + sum(y_received[w,q,1] - y_send[w,q,1] for q in 1:number_of_warehouses) + m_missing[w,1])
    ...
    @constraint(model_OiH, send_limitied_start[w in 1:number_of_warehouses, q in 1:number_of_warehouses], sum(y_send[w,q,1] for q in 1:number_of_warehouses) <= initial_stock[w])
